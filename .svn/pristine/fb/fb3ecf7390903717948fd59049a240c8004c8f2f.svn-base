        ...
        if (null == token || "".equals(token.getAccessToken()) ||  
            ...
            if (null != jsonObject) {  
                try {  
                    ...
                    token.setAccessToken(jsonObject.getString("access_token"));  
                    token.setExpiresIn(jsonObject.getInt("expires_in"));  
                    token.setAccess_token_date(new Date());
                } catch (JSONException e) {  
                    token = null;  
                    // 获取token失败  
//                    MyLog.writelogfile("获取token失败 errcode:{} errmsg:{}",  
...
//                                       jsonObject.getString("errmsg"));  
     ...
     * 获取用户信息.
     */
    public final static String user_get_url = "https://api.weixin.qq.com/cgi-bin/user/info?access_token=ACCESS_TOKEN&openid=OPENID&lang=zh_CN";
    public static JSONObject getUser(String access_token, String openid) {
    	String requestUrl = user_get_url.replace("ACCESS_TOKEN", access_token).replace("OPENID", openid);  
        // 发起GET请求获取凭证  
        JSONObject jsonObject = httpsRequest(requestUrl, "GET", null); 
        if (null != jsonObject) {
        	return jsonObject;
